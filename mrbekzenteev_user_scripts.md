...
2. Пользователь создаёт растровый слой в списке слоёв.
...
6. Пользователь перемещает первый созданный слой на передний план нажатием стрелки вверх ("↑") в списке слоёв.
...
5. Пользователь выбирает желаемую текстуру кисти (кисть, каллиграфическая ручка, карандаш).
...
6. Пользователь кликает левой кнопкой в пределах ранее выделенной области, и область выделяется выбранным цветом.
...
1. Пользователь добавляет слой-изображение (отличие от растрового слоя заключается в том, что нельзя редактировать отдельные пиксели, но можно выравнивать по сетке и работать как с единой картинкой).
2. Появляется диалоговое окно, в котором пользователь может выбрать изображение со своего компьютера, которое он хочет вставить в этот слой. Пользователь выбирает, какое изображение добавить.
3. Пользователь использует инструмент "Добавить линию сетки".
4. Появляется диалоговое окно, спрашивающее, горизонтальную или вертикальную линию сетки он хочет создать и на сколько процентов от размера всего изображения или на сколько пикселей эта линия сетки удалена от левого верхнего угла всего изображения. Пользователь заполняет информацию, повторяет пункты 3 и 4 два раза, один раз добавив вертикальную линию сетки, другой раз - горизонтальную. Теперь вся плоскость поделена на четыре области.
...
### [ Сценарий 7 - Работа с изображениями с прозрачным фоном ]

1. Пользователь добавляет слой-изображение или выбирает уже имеющийся в списке слоёв.
2. Появляется диалоговое окно, в котором пользователь может выбрать изображение со своего компьютера, которое он хочет вставить в этот слой. Пользователь выбирает, какое изображение добавить. Предположим, что фон этого изображения залит одним цветом.
3. Пользователь выбирает цвет фона, выбрав инструмент "Пипетка" и кликнув на фон изображения.
4. Пользователь выбирает инструмент "Убрать фон", и фон изображения на этом слое становится прозрачным.
5. Пользователь добавляет новый слой.
6. Пользователь перемещает этот новый слой под тот слой, у которого ранее убрали фон.
7. Всё то, что пользователь нанёс на новый слой, будет видно за слоем с убранным фоном, если оставшееся изображение его не перекрывает.

### [ Сценарий 8 - Создание, удаление, редактирование кадров и экспорт анимированного изображения в формате GIF ]

1. Пользователь создаёт новый файл, выбирая при этом разрешение изображения, которое он хочет получить, в появившемся диалоговом окне.
2. Пользователь создаёт растровый слой в списке слоёв.
3. Пользователь изменяет растровый слой, например, рисует кистью.
4. Пользователь выбирает "Перейти на следующий кадр", поскольку до этого кадр был всего один, на второй кадр копируется информация с первого.
5. Пользователь изменяет растровый слой на втором кадре, например, с использованием инструмента "Ластик" или "Кисть".
6. Пользователь возвращается на первый кадр нажатием "Перейти на предыдущий кадр" и вносит изменения в первый кадр.
7. Пользователь использует выбирает пункт меню "Экспортировать как анимированный GIF" и выбирает длительность каждого кадра, после чего появляется диалоговое окно, в котором пользователь должен выбрать место сохранения получившегося GIF-файла и его имя. Пользователь выбирает имя и место сохранения файла.

### [ Сценарий 9 - Экспорт изображений в форматах PNG, BMP, GIF ]

1. Пользователь работает с файлом способами, описанными в сценариях 1-7.
...
### [ Сценарий 10 - Регистрация и последующая авторизация на сервере, сохранение и загрузка файлов проектов с сервера ]
...
### [ Сценарий 11 -  Сохранение и загрузка файлов с компьютера пользователя ]
